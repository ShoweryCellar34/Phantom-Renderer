...
void i_prTextureCreateOnGPU(prTextureData* texture) {
    ...
    if(!texture->TBO) {
        prLogEvent(PR_EVENT_OPENGL, PR_LOG_ERROR, "i_prTextureCreateOnGPU: Failed to create texture buffer object. Aborting operation, nothing was modified");
        return;
    }
...
void i_prTextureUpdateOnGPU(prTextureData* texture) {
    ...
    if(!texture->TBO) {
        prLogEvent(PR_EVENT_OPENGL, PR_LOG_ERROR, "i_prTextureUpdateOnGPU: Failed to update texture buffer object, invalid ID. Aborting operation, nothing was modified");
        return;
    }
...
void i_prTextureUpdateBorderColorOnGPU(prTextureData* texture) {
    prLogEvent(PR_EVENT_DATA, PR_LOG_TRACE, "i_prTextureUpdateBorderColorOnGPU: Updating texture border color (R: %f G: %f B: %f A: %f)",
        texture->borderColor.a, texture->borderColor.g, texture->borderColor.b, texture->borderColor.a);
    ...
    texture->context->TextureParameterfv(texture->TBO, GL_TEXTURE_BORDER_COLOR, texture->borderColor.raw);
}
